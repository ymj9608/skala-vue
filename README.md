...
5. 디자인은 AI를 활용하여 수정했다. 자세한 내용은 `과제 8 이후 디자인 개선 결과`에서 다루었다.
...
과제 요구사항의 날씨 검색, 지도, 지역별 날씨 카드와 상세보기 기능은 유지하면서 실제 기상 관측 사이트 형태로 화면 구성을 개선했다.

#### 통합 헤더와 로고

- 구름, 기상 레이더와 돋보기를 조합한 SKALA WEATHER SEARCH 로고를 적용했다.
- 로고와 내비게이션, 날씨 단위 변경 기능을 하나의 헤더에 배치하여 상단 높이를 줄였다.
- 날씨 대시보드, 서비스 소개, 날씨 생활 팁과 커뮤니티로 이동할 수 있도록 메뉴를 구성했다.

#### 날씨 대시보드

- 실제 해안, 섬, 암석과 나무가 보이는 배경을 적용하여 기상 관측 사이트의 분위기를 표현했다.
- 지도 영역과 지역별 날씨 현황 영역의 높이를 동일하게 맞췄다.
- 지역별 날씨 카드는 스크롤로 확인할 수 있도록 구성했다.
- 카드의 날씨 아이콘, 기온, 습도, 풍속, 온도 상태와 상세보기 버튼의 배치를 통일했다.
- 카드를 선택하면 해당 지역의 대표 위치로 지도가 확대되도록 연결했다.

#### 지도와 주소 검색

- 카카오 하이브리드 지도를 적용하여 산맥, 해안과 실제 지형을 확인할 수 있도록 했다.
- 지도 위에 주요 지역의 현재 기온과 날씨 아이콘을 표시했다.
- 국내 전체 주소 목록에서 입력한 글자가 포함된 주소를 검색하고 결과 개수를 표시하도록 했다.
...
- 내비게이션에 커뮤니티 메뉴를 추가하고 게시판 이름을 `날씨 이야기`로 구성했다.
- 게시글 목록과 글 작성 화면을 분리했으며 로그인 없이 글을 작성할 수 있도록 했다.
- 작성한 브라우저에서만 수정과 삭제 버튼이 표시되고 내부 게시글 ID는 화면과 주소에 표시하지 않는다.
- 게시글은 정적 배포에서도 사용할 수 있도록 브라우저 `localStorage`에 저장한다. 같은 주소로 다시 배포해도 동일 브라우저의 데이터는 유지되지만 다른 기기와 공유되는 공용 DB는 아니다.

#### 전체 디자인

![디자인 수정 결과](hands_on/디자인수정결과.png)
...
OpenWeather 및 카카오맵 API Key는 Source Code에 직접 작성하지 않고 `.env`와 GitHub Actions Secrets를 통해 관리하였다. 이를 통해 실제 API Key가 Git Repository에 직접 포함되는 것은 방지하였다.

다만 Vite의 `VITE_*` 환경 변수는 Build 시 Client JavaScript에 포함되므로, Backend가 없는 정적 Frontend 환경에서는 Browser에서 API Key의 노출을 완전히 방지할 수 없다. 따라서 `.env`와 GitHub Actions Secrets를 사용하는 것은 Source Code 및 Git Repository에서 Key를 분리하기 위한 방법이며, Client에 전달되는 API Key 자체를 비공개로 유지하는 방법은 아니다.

본 프로젝트는 학습 및 과제 수행을 목적으로 OpenWeather API의 무료 사용 범위 내에서 서비스를 구성하였으며, 위와 같은 Client-side API Key 관리의 보안상 한계를 인지한 상태에서 사용하고 있다. 실제 서비스 환경에서 비공개로 관리해야 하는 API Key를 사용하는 경우에는 Backend 또는 Serverless Function 등의 Server-side 환경에서 API 요청을 처리하도록 구성하는 것이 적절하다.

카카오맵은 Browser 사용을 전제로 하는 JavaScript SDK Key를 사용하며, 등록된 Domain을 통해 사용할 수 있도록 구성하였다.
